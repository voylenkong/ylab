...
import local.vng.module03.orgstructure.parser.CsvParser;

import java.io.*;
import java.util.*;

import static local.vng.module03.orgstructure.parser.CsvHeaders.*;
...
public class OrgStructureParserImpl implements OrgStructureParser {
    ...
    private static Employee getEmployee(Map<String, String> record) {
        ...
        employee.setId(Long.valueOf(record.get(HEADER_ID.getDescription())));
        if (!record.get(HEADER_BOSS_ID.getDescription()).isEmpty()) {
            employee.setBossId(Long.valueOf(record.get(HEADER_BOSS_ID.getDescription())));
        }
        employee.setName(record.get(HEADER_NAME.getDescription()));
        employee.setPosition(record.get(HEADER_POSITION.getDescription()));
        return employee;
    }
