...
import io.ylab.intensive.lesson04.movie.parser.CsvHeaders;
import io.ylab.intensive.lesson04.movie.parser.CsvParser;
...
public class MovieLoaderImpl implements MovieLoader {
    ...
    private static Movie getMovie(Map<String, String> record) {
        ...
        if (!record.get(CsvHeaders.HEADER_YEAR.getDescription()).isEmpty()) {
            movie.setYear(Integer.valueOf(record.get(CsvHeaders.HEADER_YEAR.getDescription())));
        }
        if (!record.get(CsvHeaders.HEADER_LENGTH.getDescription()).isEmpty()) {
            movie.setLength(Integer.valueOf(record.get(CsvHeaders.HEADER_LENGTH.getDescription())));
        }
        movie.setTitle(record.get(CsvHeaders.HEADER_TITLE.getDescription()));
        movie.setSubject(record.get(CsvHeaders.HEADER_SUBJECT.getDescription()));
        movie.setActors(record.get(CsvHeaders.HEADER_ACTOR.getDescription()));
        movie.setActress(record.get(CsvHeaders.HEADER_ACTRESS.getDescription()));
        movie.setDirector(record.get(CsvHeaders.HEADER_DIRECTOR.getDescription()));

        if (!record.get(CsvHeaders.HEADER_POPULARITY.getDescription()).isEmpty()) {
            movie.setPopularity(Integer.valueOf(record.get(CsvHeaders.HEADER_POPULARITY.getDescription())));
        }
        if (!record.get(CsvHeaders.HEADER_AWARDS.getDescription()).isEmpty()) {
            movie.setAwards(record.get(CsvHeaders.HEADER_AWARDS.getDescription()).equals("Yes"));
        }
